...
class Encoder extends PipelineStep {
	...
	async process(items) {
		for(const item of items) {
			const outFile = await this.encode(item.path);
			if(outFile) {
	...
	async encode(path) {
		...
		if(this.minDuration && mediaInfo.general.duration < this.minDuration) {
	...
	static bestAudio(trax, lang) {
		...
		for(let i = 0; i < trax.length; i++) {
			const trac = trax[i];
			if(trac.Language === lang && trac.Default && !bestTrac.Default) {
				bestTrac = trac;
				trackNum = i;
			} else if(trac.Language === lang && !bestTrac.Default && trac.Channels > bestTrac.Channels) {
				bestTrac = trac;
				trackNum = i;
			} else if(trac.Language === lang && bestTrac.Language !== Language) {
				bestTrac = trac;
				trackNum = i;
			} else if(trac.Channels > bestTrac.Channels && bestTrac.Language !== Language) {
				bestTrac = trac;
				trackNum = i;
			}
		}
