...
class Filter extends PipelineStep {
	constructor(config) {
		super(config);
	...
	async process(items) {
		...
		for(let item of items) {
			...
			const fileName = Path.basename(item.path, ext);
			...
			// Get the final sorted name
			item.resultName = await this.getOutputFile(item.path);
			if(await Misc.fileExists(item.resultName)) {
				console.info(`Ignoring encode for file -- already exists:\n\t${fileName}`);
				deleteItems.push(item);
				continue;
			}
		...
		for(let deleteItem of deleteItems) {
			await Misc.deleteFolder(deleteItem.path, false);
		}
	...
	async lookupOutputFile(path) {
		...
		try {
			if(fileInfo.season && fileInfo.episode && this.tv) {
				...
				const seriesOptions = await this.lookup.getTVSeriesOptions(fileInfo.title, fileInfo.year);
				...
				const movieOptions = await this.lookup.getMovies(fileInfo.title, fileInfo.year);
