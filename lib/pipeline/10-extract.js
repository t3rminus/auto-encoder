			...
			if(await this.filter.checkPath(out)) {
				try {
					...
					if((+stat.size) === (+file.size)) {
						this.logSilly(`Already extracted ${Path.basename(file.name)}`);
						results.push({
							srcPath: item.path,
				...
				} catch(err) { /* ignore error-- file does not exist */ }
			...
			if(await this.filter.checkPath(out)) {
				try {
					...
					if((+stat.size) === (+file.uncompressedSize)) {
						this.logSilly(`Already extracted ${Path.basename(file.path)}`);
						results.push({
							srcPath: item.path,
							resultName: await this.filter.getOutputFile(out),
							path: out,
				...
				this.log(`Extracting ${Path.basename(file.path)}`);
				await new Promise((y,n) => {
					file.stream()
					.pipe(fs.createWriteStream(out))
					.on('error', n)
				...
				this.logSilly(`Extracted ${Path.basename(file.path)}`);
	...
	async handleFile(item) {
		...
		if(await this.filter.checkPath(out)) {
			...
			try {
				...
				const srcStat = await fs.stat(item.path);
				exists = destStat.size === srcStat.size;
			} catch(err) { /* ignore error-- file does not exist */ }
			
			if(this.delete) {
				if(exists) {
					this.log(`Already Moved ${Path.basename(item.path)}`);
				} else {
				...
				if(exists) {
					this.log(`Already Copied ${Path.basename(item.path)}`);
				} else {
