...
class EncodeWatcher extends EventEmitter {
	constructor(options) {
		...
		this.addWatcher.on('add', (path) => this.extractQueue.push(path));
		...
		this.encodeWatcher.on('add', (path) => this.encodeQueue.push(path));
	...
	extractFile(path, cb) {
		...
		this.recentExtracts[path] = setTimeout(function(){
	...
	encodeFile(path, cb) {
		...
		this.recentEncodes[path] = setTimeout(function(){
		...
		Bluebird.delay(5000)
			...
			.then((mediaInfo) => {
				...
				if(!this.opts.minDuration || mediaInfo.general.duration > this.opts.minDuration) {
					...
					settings.audio.replace(/%t/g, audioTrack.i + 1);
