...
class EncodeWatcher extends EventEmitter {
	constructor(options) {
		...
		this.encodeWatcher.on('add', (path) => {
			const fullPath = Path.join(this.encodeDir, path);
			this.db.files.insert({ file: fullPath })
			.then(() => {
	...
	_handleFile(path, cb) {
		const dest = Path.join(this.encodeDir, Path.basename(path));
		
		Bluebird.try(() => {
			return fs.access(dest, fs.constants.R_OK | fs.constants.W_OK)
			.then(() => true, () => false)
			.then((exists) => {
				if(!exists) {
