...
const compareNames = function(a, b) {
	...
	resultMax = function resultMax(value) {
	...
	rateResults = function rateResults(results, title, year) {
		...
		_.forEach(results, function(result) {
			...
			if(result.date) {
				...
				rating += recentRating(result.date) * 4;
			}
			if(rating > maxRank) {
				maxRank = rating;
			}
	...
	bestResult = function bestResult(results, title, year) {
		...
		results = rateResults(results, title, year);
		results = _.sortBy(results, 'matchProbability').reverse();
...
MediaLookup.prototype.buildMDBProxy = function MediaLookup_buildMDBProxy(options) {
	...
	return new Proxy(mdb, {
		get: (target, prop) => {
			if(_.isFunction(target[prop])) {
				return function() {
					return new Bluebird.fromCallback((cb) => {
						requestQueue.push({
							target: target,
							prop: prop,
...
MediaLookup.prototype.getTVSeriesOptions = function MediaLookup_getTVSeries(series, year) {
	return TVMaze.searchAsync(series.trim())
		.then((seriesResults) => {
			...
			return rateResults(seriesResults.map(item => this.normalizeTVMazeSeries(item)), series, year);
		});
};

MediaLookup.prototype.getTVSeries = function MediaLookup_getTVSeries(series, year, force) {
			...
			return this.mdb.searchMovie(query)
				.then((movieResults) => {
					...
					.then(function(results) {
						return rateResults(results, movieTitle, year);
					});
				});
		});
	});
