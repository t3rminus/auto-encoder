...
class MediaLookup {
	...
	async getTVSeriesOptions(series, year) {
    let seriesResults;
    try {
		  seriesResults = await TVMaze.searchAsync(encodeURIComponent(series.trim()));
    } catch (err) {
      return [];
    }
	...
	async getTVSeries(series, year, force) {
    ...
    const seriesResults = this.getTVSeriesOptions(series, year);
    if(!seriesResults && !seriesResults.length) {
      throw new NoResultError('A movie with that TVMaze ID was not found');
    }

    const theSeries = MediaLookup.bestResult(seriesResults, series, year);
    return this.tvmazeShowCache[series][year] = this.getTVSeriesInfo(theSeries.tvMazeId);
	}

	async getTVSeriesInfo(tvMazeId) {
	...
	getMovies(movieTitle, year, retry = false) {
    movieTitle = movieTitle.trim();
		return Bluebird.try(() => {
			...
			return this.mdbConfigCache.then((mdbConfig) => {
				...
				.then((movieResults) => {
					if(!movieResults.results.length && !retry && /[0-9]{4}$/.test(movieTitle)) {
            return this.getMovies(movieTitle.replace(/[0-9]{4}$/, '').trim(), year, true);
					} else if(!movieResults.results.length) {
            return [];
          }
		...
		return Bluebird.try(() => {
			...
			return seriesPromise.then(series => {
				...
				if(!this.xemCache[series.tvdbId]) {
					this.xemCache[series.tvdbId] = fetch(`http://thexem.de/map/all?id=${+series.tvdbId}&origin=tvdb`)
					.then(r => r.json());
				}
